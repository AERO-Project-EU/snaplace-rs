mod common;

use std::collections::HashMap;

use anyhow::Context;
use bytes::Bytes;
use const_format::formatcp;
use tracing_test::traced_test;

use crate::common::{check_request, check_response, issue_cold, issue_warm};

const NAME: &str = "aerofb_matmul";

#[::tokio::test]
#[traced_test]
async fn aerofb_matmul() -> ::anyhow::Result<()> {
    //
    // TODO(ckatsak): fork/exec:
    //      `docker run --rm -it --pull=always -p 8000:8000 ghcr.io/aero-project-eu/aerofb-matmul:0.0.1`
    // or just do it manually before running the test?
    //

    // Create request
    let handler_payload =
        ::serde_json::to_vec::<HashMap<_, _>>(&HashMap::from_iter([("N", 512), ("M", 512)]))
            .context("failed to serialize handler's payload")?;
    let agent_payload =
        ::serde_json::to_vec::<HashMap<_, _>>(&HashMap::from_iter([("payload", handler_payload)]))
            .context("failed to serialize agent's payload")?;
    let mut sreq = ::snaplace_grpc::Request {
        invocation_id: formatcp!("{NAME}-cold").into(),
        function_id: NAME.into(),
        metadata_map: Default::default(),
        payload: Bytes::from(agent_payload),
    };
    //let mut sreq = common::create_snaplace_grpc_request(NAME, &handler_payload)?;
    check_request(sreq.clone())?;

    let sresp = issue_cold::<::snaplace_grpc::Response>(sreq.clone()).await?;
    check_response(sresp)?;

    sreq.invocation_id = formatcp!("{NAME}-warm").into();
    let sresp = issue_warm::<::snaplace_grpc::Response>(sreq).await?;
    check_response(sresp)?;

    Ok(())
}

#[cfg(test)]
mod tests {
    use serde_json::Value as serdeValue;

    #[test]
    fn json() {
        let raw_handler_payload = [
            123, 34, 78, 34, 58, 32, 53, 49, 50, 44, 32, 34, 77, 34, 58, 32, 53, 49, 50, 125,
        ];
        let deser_handler_payload = ::serde_json::from_slice::<serdeValue>(&raw_handler_payload)
            .expect("deserialize HANDLER");
        eprintln!("{deser_handler_payload:?}");

        //let raw_agent_payload = r#"{"payload": [123, 34, 78, 34, 58, 32, 53, 49, 50, 44, 32, 34, 77, 34, 58, 32, 53, 49, 50, 125]}"#;
        let raw_agent_payload = format!("{{\"payload\": {raw_handler_payload:?}}}");
        let deser_agent_payload =
            ::serde_json::from_str::<serdeValue>(&raw_agent_payload).expect("deserialize AGENT");
        eprintln!("{deser_agent_payload:?}");
    }
}
