...
const NAME: &str = "aerofb_cnn_serving";

#[::tokio::test]
#[traced_test]
async fn aerofb_cnn_serving() -> ::anyhow::Result<()> {
    //
    // TODO(ckatsak): fork/exec:
    //      `docker run --rm -it --pull=always -p 8000:8000 ghcr.io/aero-project-eu/aerofb-cnn_serving:0.0.1`
    // or just do it manually before running the test?
    //

    // Create request
    let handler_payload =
        ::serde_json::to_vec::<HashMap<_, _>>(&HashMap::from_iter([("img_idx", 0)]))
