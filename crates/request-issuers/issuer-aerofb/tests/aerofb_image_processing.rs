...
const NAME: &str = "aerofb_image_processing";

#[::tokio::test]
#[traced_test]
async fn aerofb_image_processing() -> ::anyhow::Result<()> {
    //
    // TODO(ckatsak): fork/exec:
    //      `docker run --rm -it --pull=always -p 8000:8000 ghcr.io/aero-project-eu/aerofb-image_processing:0.0.1`
    ...
    let handler_payload = ::serde_json::to_vec::<HashMap<_, _>>(&HashMap::from_iter([
        ...
        (
            "ops",
            &["filter", "flip", "gray_scale", "resize", "rotate"].join("-"),
        ),
    ]))
